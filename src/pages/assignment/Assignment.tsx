...
const Assignment = () => {
  ...
  const { data: routeShipments, isPending: routeShipmentsPending } = useQuery({
    ...
    queryFn: ({ signal }) => getAllShipmentsByAssignmentId(assignmentSelectedId, signal),
    ...
    <>
      <div className="flex flex-1">
        <div className="sticky top-(--header-height) max-h-[calc(100vh-(var(--header-height)))] w-90 shrink-0 p-3">
          <div className="flex flex-col justify-baseline border border-gray-300 h-full min-h-0 rounded-md overflow-hidden">
            <div className="flex justify-between items-center px-3 py-2 border-b border-gray-300 bg-gray-200">
              <h2 className="uppercase font-mono">Assignments</h2>
              <Button
                size="sm"
            ...
            {showCreateForm && <AssignmentCreateForm onClose={() => setShowCreateForm(false)} />}
            <AssignmentList />
          </div>
        </div>
        <div className="sticky top-(--header-height) max-h-[calc(100vh-(var(--header-height)))] w-96 shrink-0 flex flex-col gap-3 p-3 min-h-0">
